...
/** Shared card shell — same color treatments as recipient stat cards */
function StatShell({
  tone,
  children,
}: {
...
// -- THE THREE TIPPER CARDS ---------------------------------------------

export function LifetimeGivenCard({
  amountCents,
  tipCount,
  uniqueRecipients,
  sinceISO,
  ...
  const sinceLabel = sinceISO
    ? new Date(sinceISO + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })
    : '';
  return (
    <StatShell tone="jade">
      <Eyebrow tone="jade">Lifetime given</Eyebrow>
      ...
      <p className="mt-2 text-sm text-ink-dim">
        {hasData ? (
          <>
            ...
            {' '}to <strong className="font-mono font-semibold text-jade-500">{uniqueRecipients} unique recipients</strong>
            {sinceLabel && <> · since {sinceLabel}</>}
          </>
        ) : (
          <>Your first tip starts the count.</>
        )}
      </p>
      <div className="mt-4">
...
export function ThisMonthGivenCard({
  amountCents,
  tipCount,
  percentVsLastMonth,
  bars,
  ...
  return (
    <StatShell tone="gold">
      <Eyebrow tone="gold">This month given</Eyebrow>
      ...
      <p className="mt-2 text-sm text-ink-dim">
        {hasData ? (
          ...
          <>Your monthly rhythm appears once you tip.</>
        )}
      </p>
      <div className="mt-4">
...
export function TipperAppreciationStreakCard({
  weeks,
  sinceLabel,
}: {
  weeks: number;
    ...
    <StatShell tone="coral">
      ...
      <p className="mt-2 text-sm text-ink-dim">
        {hasData
          ...
          : 'Tip once this week to start your streak.'}
      </p>
      <div className="mt-4">
