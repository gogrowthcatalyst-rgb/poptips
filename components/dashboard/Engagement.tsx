...
import type { Achievement, RecentTip } from '@/lib/dashboard/placeholder-data';
      ...
      <div className="relative grid items-center gap-6 md:grid-cols-[1fr_auto] md:gap-8">
        <div>
          ...
          <h3 className="mt-2 font-display text-3xl font-medium leading-tight tracking-tightest text-paper md:text-4xl">
            You just crossed <em className="italic text-gold-100">{milestone}.</em>
          </h3>
        ...
        <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))' }}>
          {achievements.map((a) => (
            ...
            >
              ...
              <p className="font-display text-sm font-medium italic text-ink">
                {a.name}
              </p>
    ...
    <div className="relative overflow-hidden rounded-3xl border border-line-soft bg-paper p-6 shadow-lift md:p-8">
      <div className="mb-4 flex items-start justify-between gap-3">
        <h3 className="font-display text-xl font-medium leading-snug text-ink md:text-2xl">
          Recent <em className="italic text-jade-500">appreciation.</em>
        </h3>
        <button
          type="button"
      ...
      {tips.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-2 py-12 text-center">
          ...
          <p className="font-display text-xl font-medium italic text-ink-dim">
            Your first tip will land here.
          </p>
          <p className="max-w-md text-sm text-ink-faint">
            Share your QR code, your handle, or the link. Every tip you receive shows
            up in this list — with the amount, the app, and the time.
          </p>
        </div>
      ) : (
        <ul className="flex flex-col divide-y divide-line-soft">
          {tips.map((tip, i) => {
            ...
            return (
              <li key={i}>
                <div className="grid grid-cols-[36px_1fr_auto_auto] items-center gap-3 py-3 px-1">
                  <div
                  ...
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium text-ink">{tip.name}</p>
